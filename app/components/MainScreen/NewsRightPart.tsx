...
const toRecipeDraft = (value: unknown): RecipeDraft | null => {
  ...
  const source = (root.recipe && typeof root.recipe === "object" ? root.recipe : root) as Record<string, unknown>;
    ...
    return input
      .map((item) => {
        ...
        if (item && typeof item === "object") {
          ...
          if (typeof row.description === "string") return row.description.trim();
  ...
  return {
    ...
    cooking_time: typeof source.cooking_time === "number" ? source.cooking_time : undefined,
    calorific: typeof source.calorific === "number" ? source.calorific : undefined,
...
const summarizeAiResponse = (value: unknown): string => {
  if (!value) return "Пока не удалось получить ответ. Попробуйте уточнить запрос.";
...
export default function RightPart() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: "welcome",
      role: "assistant",
      text: "Привет! Я микро-шеф. Напишите продукты через запятую, и я соберу рецепт.",
    },
  ]);
    ...
    const loadPopularAuthors = async () => {
      try {
        ...
        const sorted = Array.from(byAuthor.values())
          .sort((a, b) => b.likesCount + b.recipesCount - (a.likesCount + a.recipesCount))
          .slice(0, 5);
  ...
  const handleSaveAiRecipeToFavorites = async (title: string) => {
    try {
      ...
      if (!first) {
        alert("Рецепт не найден в базе. Сначала сохраните его как обычный рецепт.");
        return;
      }
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const response = await aiService.generateByProducts(products.length > 0 ? products : [userText]);
      ...
      <div className="flex w-full flex-col gap-4">
        {isAuthenticated && (
          <div className="rounded-[15px] border border-[#eaeaea] bg-white p-3">
            <p className="font-nunito text-base font-bold text-umami-dark-gray">Микро-шеф</p>
            <p className="mt-1 font-inter text-sm text-umami-gray">Подскажет рецепт по вашим продуктам.</p>
            <button
              type="button"
        ...
        <div className="rounded-[15px] border border-[#eaeaea] bg-white p-3">
          <div className="mb-2 flex items-center justify-between">
            <p className="font-nunito text-base font-bold text-umami-dark-gray">Популярные авторы</p>
          </div>
          ...
          {isLoadingAuthors ? (
            ...
            <div className="flex flex-col gap-2.5">
              {popularAuthors.map((author) => {
                ...
                return (
                  <Link key={author.id} href={`/users/${author.id}`} className="rounded-[10px] border border-[#ececec] p-2.5 transition-colors hover:bg-[#fcfaf5]">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="truncate font-nunito text-sm font-bold text-umami-dark-gray">{author.name}</p>
                        <p className="truncate font-inter text-xs text-umami-gray">@{author.username}</p>
                      </div>
                    </div>
          ...
          <div className="grid h-[80vh] w-full max-w-[1080px] grid-cols-[minmax(0,1fr)_320px] gap-5">
            <div className="flex h-full flex-col rounded-[20px] border border-[#eaeaea] bg-white p-4">
              <div className="mb-3 flex items-center justify-between">
                <h3 className="font-nunito text-xl font-bold text-umami-dark-gray">Чат с микро-шефом</h3>
                <button
                  type="button"
              ...
              <div className="flex-1 space-y-2 overflow-y-auto rounded-2xl border border-[#efefef] bg-[#faf9f6] p-3">
                {messages.map((message) => (
                  <div key={message.id} className={`${message.role === "user" ? "ml-auto" : ""} max-w-[85%]`}>
                    {message.recipeDraft ? (
                      <div className="rounded-2xl border border-[#e9e3d3] bg-white px-3 py-3 text-umami-dark-gray">
                        <p className="font-nunito text-lg font-bold">{message.recipeDraft.title}</p>
                        <p className="mt-1 text-sm text-umami-gray">{message.recipeDraft.description}</p>
                        <p className="mt-2 text-xs text-umami-gray">
                          {message.recipeDraft.portion ? `${message.recipeDraft.portion} порц. • ` : ""}
                          {message.recipeDraft.cooking_time ? `${message.recipeDraft.cooking_time} мин • ` : ""}
                          {message.recipeDraft.difficulty || "без уровня"}
                        </p>
                        {message.recipeDraft.ingredients.length > 0 && (
                          <div className="mt-3">
                            <p className="font-nunito text-sm font-bold">Ингредиенты</p>
                            <ul className="mt-1 list-disc pl-4 text-sm">
                              {message.recipeDraft.ingredients.slice(0, 6).map((item, idx) => (
                                <li key={`${message.id}-ing-${idx}`}>{item}</li>
                              ))}
                            </ul>
                          </div>
                        )}
                        {message.recipeDraft.steps.length > 0 && (
                          <div className="mt-3">
                            <p className="font-nunito text-sm font-bold">Шаги</p>
                            <ol className="mt-1 list-decimal pl-4 text-sm">
                              {message.recipeDraft.steps.slice(0, 4).map((item, idx) => (
                                <li key={`${message.id}-step-${idx}`}>{item}</li>
                              ))}
                            </ol>
                          </div>
                        )}
                        <div className="mt-3 flex gap-2">
                          <button
                            type="button"
                            onClick={() => void handleSaveAiRecipeToFavorites(message.recipeDraft!.title)}
            ...
            <aside className="h-full rounded-[20px] border border-[#eaeaea] bg-white p-4">
              <p className="font-nunito text-base font-bold text-umami-dark-gray">Как спросить</p>
